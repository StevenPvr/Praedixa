...
Dans ce rapport exhaustif signe, seuls les eches `critical` / `high` / `medium` restent bloquants pour le `pre-push`; les eches `low` doivent rester traces dans le rapport (`low_failed_checks`) et dans la sortie `PASS with warnings`, sans casser la livraison a eux seuls.
