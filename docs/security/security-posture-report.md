...
**Date**: 2026-02-08
**Version**: 2.0 (finale — integre tous les audits agents)
...
| P0-2 | render.yaml manquant vars production (KEY*PROVIDER, SCW*\*) | watchtower | **FIXE** |

### 2.2 P1 -- Hauts

| #    | Finding                                                 | Agent      | Fichier                                  | OWASP        |
| ---- | ------------------------------------------------------- | ---------- | ---------------------------------------- | ------------ | ----------------- |
| P1-1 | Admin org update accepte `body: dict` (mass assignment) | sentinel   | `admin_orgs.py:188`                      | API6         |
| P1-2 | Rate limiting IP spoofing via X-Forwarded-For           | sentinel   | `rate_limit.py:49`                       | API4         |
| P1-3 | Endpoints erasure sans `log_admin_action()`             | counsel    | `admin.py` (5 endpoints)                 | --           |
| P1-4 | GitHub Actions non pinnes par SHA                       | watchtower | `.github/workflows/`                     | Supply Chain |
| P1-5 | ~~CSP nonce-based pour les 3 frontends~~                | bastion    | apps/\*\*/middleware.ts                  | A05          | **FIXE**          |
| P1-6 | `recurrence_pattern` non masque (Art. 9)                | counsel    | `medical_masking.py`                     | --           |
| P1-7 | Rate limiting landing in-memory (reset au redeploy)     | bastion    | `landing/api/pilot-application/route.ts` | API4         | **ACCEPTED RISK** |
...
| P2-11 | Messages d'erreur login exposent details Supabase         | bastion          | `webapp+admin/(auth)/login/page.tsx`    |
| P2-12 | CSP Violation Reporting absent (`report-uri`/`report-to`) | bastion          | `apps/*/lib/security/csp.ts`            |
...
| P3-8  | Bandit severity inconsistante CI vs pre-commit             | watchtower | -ll vs -lll                              |
| P3-9  | Branch protection non configuree                           | watchtower | Direct push possible                     |
| P3-10 | `encodeURIComponent` manquant dans API_ENDPOINTS constants | bastion    | `webapp/lib/api/endpoints.ts`            |
| P3-11 | Client API propage `error.details` au DOM                  | bastion    | `webapp+admin/lib/api/client.ts`         |
| P3-12 | Email templates HTML inline (pas de lib templating)        | bastion    | `landing/api/pilot-application/route.ts` |
...
| A05 | Security Misconfiguration | **FIXE**     | DEBUG off en prod, CSP nonce-based deploye, HSTS + X-Frame-Options + CORP + COOP via middleware. GH Actions non pinnes (P1-4)                                                                   |
...
| A08 | Software & Data Integrity | **PARTIEL**  | HMAC sur fit_parameters, `strict-dynamic` CSP. GH Actions non SHA-pinnes (P1-4)                                                                                                                 |
...
| 2   | ~~P0-2: render.yaml vars production~~                            | ~~Done~~       | watchtower |
...
| 6   | P1-4: Pinner GitHub Actions par SHA                              | S              | watchtower |
...
| 21  | P3-9: Configurer GitHub branch protection                             | S      | watchtower |
| 22  | P3-8: Aligner bandit severity CI = pre-commit                         | XS     | watchtower |
...
| Infrastructure   | 7/10   | Docker harden, CI gates. -1 GH Actions, -1 branch protection, -1 monitoring                                                                                        |
| Supply Chain     | 7/10   | Dependabot, frozen locks, pip-audit. -2 GH Actions non SHA, -1 Docker Dependabot                                                                                   |
...
| **Infrastructure** | Moyenne  | Docker hardened, CI gates, frozen lockfiles                               | GH Actions non SHA, branch protection, monitoring          |
...
| render.yaml vars     | watchtower | `render.yaml`                             |
| Dockerfile hardening | watchtower | `Dockerfile`, `docker-compose.yml`        |
| .gitignore secrets   | watchtower | `.gitignore`                              |
| CSP nonce-based      | bastion    | `middleware.ts` (3 apps)                  |

### Methodologie

- Revue statique de code de tous les fichiers dans `apps/api/app/` (core, models, services, routers)
- Revue statique frontend : `apps/webapp`, `apps/admin`, `apps/landing`, `packages/ui` (XSS, CSP, auth, redirections)
...
- Audit CI/CD, Docker, supply chain, secrets scanning
