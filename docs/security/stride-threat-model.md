...
**Scope**: Backend API (FastAPI), Frontend (Next.js 15), Infrastructure (Cloudflare Workers + Scaleway)
...
- **Auth**: Supabase JWT (RS256/ES256/EdDSA en prod, HS256 en dev uniquement)
...
| S2  | **Vol de token JWT** : XSS extrait le token du localStorage ou des cookies         | 5      | 3     | Tokens geres par Supabase SSR (httpOnly cookies) ; pas de localStorage pour les tokens                                                                  | Implementer CSP nonce-based pour bloquer les scripts injectes. Ajouter `Secure; SameSite=Strict` sur tous les cookies de session.       |
| S3  | **Session hijacking** via token vole en transit                                    | 4      | 1     | HTTPS force (Cloudflare) ; tokens ephemeres Supabase (1h par defaut)                                                                                    | CONFORME. Considerer la reduction du TTL JWT a 15-30min avec refresh token rotation.                                                    |
| S4  | **Usurpation d'admin** : attaquant modifie le claim `role` dans le JWT             | 5      | 1     | Role extrait de `app_metadata` (admin-only writable dans Supabase) ; `user_metadata` ignore en prod ; validation contre `_KNOWN_ROLES`                  | CONFORME. Le `app_metadata` ne peut etre modifie que server-side par Supabase.                                                          |
...
| T2  | **Modification de donnees en transit**                                          | 4      | 1     | HTTPS force (Cloudflare Workers) ; pas de HTTP en prod                                                                              | CONFORME.                                                                                                                                                                                      |
...
| D4  | **Slowloris / connection exhaustion** | 3      | 2     | Cloudflare Workers gerent les connexions entrantes (protection native)                               | CONFORME grace a Cloudflare. Verifier les timeouts sur les connexions asyncpg vers PostgreSQL.                                                                           |
...
| #   | Menace                                                                                  | Impact | Proba | Controles existants                                                                                                          | Recommandations                                                                                                                                                               |
| --- | --------------------------------------------------------------------------------------- | ------ | ----- | ---------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| E1  | **Escalade de role** : utilisateur modifie son role dans le JWT                         | 5      | 1     | Role extrait de `app_metadata` (admin-only dans Supabase) ; `_KNOWN_ROLES` validation ; role inconnu -> downgrade a "viewer" | CONFORME. Un utilisateur ne peut pas modifier `app_metadata` via les SDK client Supabase.                                                                                     |
...
- Securite physique de l'infrastructure Scaleway/Cloudflare
- Vulnerabilites dans les dependances tierces (couvert par pip-audit et gitleaks dans pre-commit)
- Ingenierie sociale
- DDoS reseau (couvert par Cloudflare)
