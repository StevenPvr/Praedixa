...
usage() {
  cat >&2 <<'EOF'
Usage: ./scripts/scw-rollback-plan.sh \
...
Deploy command:
  ./scripts/scw-release-deploy.sh --manifest "$rollback_manifest" --env "$(printf '%s' "$plan_json" | jq -r '.environment')" --services "$services_csv"
EOF
}
