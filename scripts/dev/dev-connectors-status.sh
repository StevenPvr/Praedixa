...
if [[ -f "${PID_FILE}" ]]; then
  ...
  if [[ -n "${pid}" ]] && is_process_alive "${pid}" && is_tcp_port_open "${PORT}"; then
    echo "[dev:connectors] running (pid ${pid}) on http://localhost:${PORT}"
    exit 0
  fi
...
if is_tcp_port_open "${PORT}"; then
  echo "[dev:connectors] running on http://localhost:${PORT} (unmanaged process)"
  exit 0
fi

echo "[dev:connectors] not running"
exit 1
