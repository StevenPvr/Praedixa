...
sync_domain_binding_and_dns() {
  ...
  if [ -z "$existing_domain" ]; then
    ...
    if ! scw container domain create hostname="$CENTAURUS_HOSTNAME" container-id="$container_id" region="$REGION" >/dev/null; then
      echo "[warn] container domain binding could not be completed yet for $CENTAURUS_HOSTNAME" >&2
    fi
  else
    ...
    if [ "$existing_container_id" != "$container_id" ]; then
      ...
      if ! scw container domain create hostname="$CENTAURUS_HOSTNAME" container-id="$container_id" region="$REGION" >/dev/null; then
        echo "[warn] container domain rebinding could not be completed yet for $CENTAURUS_HOSTNAME" >&2
      fi
    else
