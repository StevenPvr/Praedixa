...
CI_PYTHON_TOOL="${ROOT_DIR}/scripts/gates/ci-python-tool.sh"
...
run_precommit_layer() {
  ...
  run_check "layer-a:precommit-blocking" "security" "high" "./scripts/gates/gate-precommit-blocking.sh"
}

run_prepush_layer() {
  run_check "policy:exceptions" "governance" "high" "python3 scripts/validate-security-exceptions.py --quiet"
  run_check "layer-b:prepush-deep" "security" "high" "./scripts/gates/gate-prepush-deep.sh"
}
...
run_manual_exhaustive_layer() {
  ...
  run_check "quality:static-monorepo" "quality" "medium" "./scripts/gates/gate-quality-static.sh"
  run_check "performance:budget-baselines" "quality" "medium" "pnpm performance:validate-budgets"
  run_check "tests:vitest-coverage" "quality" "medium" "pnpm test:coverage"
  run_check "tests:api-ts" "quality" "medium" "pnpm --filter @praedixa/api-ts test"
  run_check "tests:pytest-data" "quality" "medium" "cd app-api && uv run pytest tests/"
  run_check "quality:alembic-heads" "quality" "low" "./scripts/check-alembic-heads.sh"
  run_check "build:next-monorepo" "quality" "high" "pnpm build"
...
./scripts/gates/gate-report-sign.sh --unsigned "$UNSIGNED_REPORT" --output "$REPORT_PATH" >/dev/null
