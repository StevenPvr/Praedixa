...
usage() {
  cat <<'EOF' >&2
Usage: scripts/ci-evaluate-scope.sh \
  --workflow <api|admin> \
  --event-name <event> \
  --base-sha <sha> \
  --head-sha <sha> \
...
CRITICAL_CI_SCRIPT_GROUP='scripts/(ci-[^/]+\.sh|gate-exhaustive-local\.sh|verify-gate-report\.sh|release-manifest-(create|sign|verify)\.sh|scw-release-(build|deploy|manifest-create|promote)\.sh|scw-preflight-(deploy|staging)\.sh|scw-post-deploy-smoke\.sh|scw-deploy-(landing|frontend|api|auth)\.sh|scw-configure-(landing|frontend|api|auth|centaurus|skolae)-env\.sh)'
...
API_DATA_ENGINE_SCOPE="^(\\.github/workflows/ci-api\\.yml|${ROOT_CONFIG_GROUP}|scripts/ci-python-tool\\.sh|app-api/)"
