...
GREEKIA_HOSTNAME="${GREEKIA_HOSTNAME:-greekia.praedixa.com}"
NAMESPACE_NAME="greekia-prod"
CONTAINER_NAME="greekia-prospect"
DNS_MODE="${SCW_BOOTSTRAP_DNS_MODE:-external-pending}"
...
validate_dns_mode() {
  case "$DNS_MODE" in
    scaleway-managed|external-verified|external-pending) ;;
    *)
      echo "Unsupported SCW_BOOTSTRAP_DNS_MODE: ${DNS_MODE}" >&2
      echo "Supported values: scaleway-managed, external-verified, external-pending" >&2
      exit 1
      ;;
  esac
}

if [[ "$GREEKIA_HOSTNAME" != *.praedixa.com ]]; then
  echo "GREEKIA_HOSTNAME must be under praedixa.com (current: $GREEKIA_HOSTNAME)" >&2
  exit 1
fi

DNS_RECORD_NAME="${GREEKIA_HOSTNAME%.praedixa.com}"
...
ensure_domain_binding_and_dns() {
  ...
  existing_domain="$(scw container domain list region="$REGION" -o json | jq -c --arg h "$GREEKIA_HOSTNAME" '.[] | select(.hostname==$h)' | head -n1)"
  if [ -z "$existing_domain" ]; then
    echo "[create] container domain binding $GREEKIA_HOSTNAME -> $CONTAINER_NAME"
    scw container domain create hostname="$GREEKIA_HOSTNAME" container-id="$container_id" region="$REGION" >/dev/null
  else
    ...
    if [ "$existing_container_id" != "$container_id" ]; then
      ...
      echo "[rebind] container domain $GREEKIA_HOSTNAME to $CONTAINER_NAME"
      scw container domain delete "$existing_domain_id" region="$REGION" >/dev/null
      scw container domain create hostname="$GREEKIA_HOSTNAME" container-id="$container_id" region="$REGION" >/dev/null
    else
      echo "[skip] container domain $GREEKIA_HOSTNAME already bound to $CONTAINER_NAME"
    fi
  fi

  for _ in $(seq 1 "$VERIFY_ATTEMPTS"); do
    local binding_json bound_container_id binding_status
    binding_json="$(scw container domain list region="$REGION" -o json | jq -c --arg h "$GREEKIA_HOSTNAME" '.[] | select(.hostname==$h)' | head -n1)"
    ...
    if [ "$bound_container_id" = "$container_id" ]; then
      if [ -z "$binding_status" ] || [ "$binding_status" = "ready" ] || [ "$binding_status" = "pending" ]; then
        echo "[verified] container domain $GREEKIA_HOSTNAME bound to $CONTAINER_NAME"
        break
      fi
      if [ "$binding_status" = "error" ]; then
        echo "Container domain binding for $GREEKIA_HOSTNAME is in error state" >&2
        exit 1
      fi
    fi

    if [ "$_" -eq "$VERIFY_ATTEMPTS" ]; then
      echo "Container domain binding for $GREEKIA_HOSTNAME was not observable after verification window" >&2
      exit 1
    fi
  ...
  elif [ "$DNS_MODE" = "external-verified" ]; then
    echo "[verify] external DNS CNAME ${DNS_RECORD_NAME} -> $domain_name"
  else
    echo "[pending] external DNS CNAME ${DNS_RECORD_NAME} -> $domain_name"
    echo "Create this CNAME at your DNS provider, then rerun with SCW_BOOTSTRAP_DNS_MODE=external-verified if you want end-to-end verification."
    return
  fi

  for _ in $(seq 1 "$VERIFY_ATTEMPTS"); do
    local public_cname_target
    public_cname_target="$(resolve_public_cname_target "$GREEKIA_HOSTNAME" | tr -d '[:space:]')"
    if [ -n "$public_cname_target" ] && [ "$(normalize_dns_target "$public_cname_target")" = "$domain_name" ]; then
      echo "[verified] public DNS CNAME ${GREEKIA_HOSTNAME} -> $domain_name"
      return
    fi

    if [ "$_" -eq "$VERIFY_ATTEMPTS" ]; then
      echo "Public DNS for $GREEKIA_HOSTNAME does not resolve to expected CNAME target ${domain_name}" >&2
...
echo "Scaleway greekia bootstrap completed."
