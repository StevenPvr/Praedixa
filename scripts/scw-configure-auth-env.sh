...
export KEYCLOAK_SMTP_FROM KEYCLOAK_SMTP_FROM_DISPLAY_NAME KEYCLOAK_SMTP_REPLY_TO KEYCLOAK_SMTP_REPLY_TO_DISPLAY_NAME KEYCLOAK_SMTP_HOST KEYCLOAK_SMTP_PORT KEYCLOAK_SMTP_USER KEYCLOAK_SMTP_AUTH KEYCLOAK_SMTP_STARTTLS KEYCLOAK_SMTP_SSL
write_json_from_env \
  "$ENV_FILE_PATH" \
  KC_DB \
  KC_DB_URL_HOST \
...
export KC_DB_PASSWORD KC_BOOTSTRAP_ADMIN_PASSWORD KEYCLOAK_SMTP_PASSWORD
write_json_from_env \
  "$SECRETS_FILE_PATH" \
  KC_DB_PASSWORD \
...
./scripts/keycloak-ensure-email-config.sh >/dev/null

KEYCLOAK_SERVER_URL="https://${AUTH_HOSTNAME}" \
KEYCLOAK_ADMIN_USERNAME="$KEYCLOAK_ADMIN_USERNAME" \
KEYCLOAK_ADMIN_PASSWORD="$KEYCLOAK_ADMIN_PASSWORD" \
./scripts/keycloak-ensure-email-theme.sh >/dev/null

echo "Scaleway auth runtime configuration updated, including Keycloak SMTP and email theme realm config."
