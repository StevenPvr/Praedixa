...
AUTH_URL=""
CONNECTORS_URL=""
WEBAPP_SESSION_COOKIE_FILE="${WEBAPP_SESSION_COOKIE_FILE:-}"
ADMIN_SESSION_COOKIE_FILE="${ADMIN_SESSION_COOKIE_FILE:-}"
...
usage() {
  echo "Usage: $0 --env <staging|prod> [--services <comma-separated>] [--landing-url <url>] [--auth-url <url>] [--connectors-url <url>] [--webapp-session-cookie-file <path>] [--admin-session-cookie-file <path>] [--timeout <seconds>]" >&2
  echo "Note: staging webapp/admin smoke requires --auth-url to validate the real OIDC login redirect." >&2
  echo "Note: webapp/admin smoke also requires session cookie files containing a live prx_*_sess cookie header value to prove authenticated /auth/session." >&2
  exit 2
}
...
while (($# > 0)); do
  case "$1" in
    ...
    --webapp-session-cookie-file)
      WEBAPP_SESSION_COOKIE_FILE="${2:-}"
      shift 2
      ;;
    --admin-session-cookie-file)
      ADMIN_SESSION_COOKIE_FILE="${2:-}"
      shift 2
      ;;
    --timeout)
      TIMEOUT_SECONDS="${2:-}"
...
resolve_frontend_session_cookie_file() {
  local service="$1"
  case "$service" in
    webapp)
      echo "$WEBAPP_SESSION_COOKIE_FILE"
      ;;
    admin)
      echo "$ADMIN_SESSION_COOKIE_FILE"
      ;;
    *)
      echo ""
      ;;
...
resolve_frontend_session_cookie_name() {
  local service="$1"
  case "$service" in
    webapp)
      echo "prx_web_sess"
...
read_session_cookie_header() {
  local label="$1"
  local cookie_file="$2"
  local expected_cookie_name="$3"

  if [[ -z "$cookie_file" ]]; then
    echo "${label}: missing session cookie file; pass --${label%% *}-session-cookie-file or export the matching *_SESSION_COOKIE_FILE env." >&2
    return 1
  fi

  if [[ ! -f "$cookie_file" ]]; then
    echo "${label}: session cookie file not found: $cookie_file" >&2
    return 1
  fi

  python3 - "$label" "$cookie_file" "$expected_cookie_name" <<'PY'
from pathlib import Path
import sys

label, cookie_file, expected_cookie_name = sys.argv[1:4]
content = Path(cookie_file).read_text(encoding="utf-8").strip()
if not content:
    raise SystemExit(f"{label}: session cookie file is empty")
...
if not parts:
    raise SystemExit(f"{label}: session cookie file does not contain a cookie header value")

if not any(part.startswith(f"{expected_cookie_name}=") for part in parts):
    raise SystemExit(
        f"{label}: session cookie file must include {expected_cookie_name}=..."
    )
...
validate_authenticated_session_body() {
  local service="$1"
  local body_path="$2"

  python3 - "$service" "$body_path" <<'PY'
import json
import sys

service, body_path = sys.argv[1:3]
with open(body_path, "r", encoding="utf-8") as handle:
    payload = json.load(handle)

user = payload.get("user")
if not isinstance(user, dict):
    raise SystemExit("response body must contain a user object")

for field in ("id", "email", "role"):
    value = user.get(field)
    if not isinstance(value, str) or not value.strip():
        raise SystemExit(f"user.{field} must be a non-empty string")
...
if organization_id is not None and not isinstance(organization_id, str):
    raise SystemExit("user.organizationId must be a string or null")
if site_id is not None and not isinstance(site_id, str):
    raise SystemExit("user.siteId must be a string or null")
...
if service == "admin":
    ...
    if not isinstance(permissions, list) or not all(
        ...
        raise SystemExit("admin session response must contain a non-empty string[] user.permissions")
PY
}
...
run_frontend_authenticated_session_check() {
  local service="$1"
  local base_url="$2"
  local cookie_file="$3"
  local label="$4"
  local session_url="${base_url%/}/auth/session"
  local expected_cookie_name cookie_header temp_dir headers_path body_path response actual_status effective_url

  printf "  %-34s" "$label"

  if ! expected_cookie_name="$(resolve_frontend_session_cookie_name "$service")"; then
    json_log::emit \
      error \
      post_deploy_smoke.frontend_session_authenticated.failed \
      "Authenticated frontend session smoke is unsupported for service" \
      smoke_service="$service" \
      check_label="$label" \
      target_url="$session_url" \
      status=failed \
  ...
  if ! cookie_header="$(read_session_cookie_header "${service}" "$cookie_file" "$expected_cookie_name")"; then
    json_log::emit \
      error \
      post_deploy_smoke.frontend_session_authenticated.failed \
      "Authenticated frontend session smoke is missing a usable session cookie" \
      smoke_service="$service" \
      check_label="$label" \
      target_url="$session_url" \
      status=failed \
      reason=missing_session_cookie
    echo "FAIL (missing session cookie)"
    record_fail
    return
  fi
  ...
  if ! response="$(
    ...
    json_log::emit \
      error \
      post_deploy_smoke.frontend_session_authenticated.failed \
      "Authenticated frontend session smoke request failed" \
      smoke_service="$service" \
      check_label="$label" \
      target_url="$session_url" \
      status=failed \
  ...
  if [[ "$actual_status" != "200" ]]; then
    ...
    json_log::emit \
      error \
      post_deploy_smoke.frontend_session_authenticated.failed \
      "Authenticated frontend session smoke returned an unexpected status" \
      smoke_service="$service" \
      check_label="$label" \
      target_url="$session_url" \
      effective_url="$effective_url" \
      ...
      status=failed \
      reason=unexpected_status
    echo "FAIL (expected 200, got $actual_status)"
    record_fail
    return
  fi
  ...
  if ! validate_https_url "$label effective URL" "$effective_url" "$(url_hostname "$base_url")" "/auth/session"; then
    ...
    json_log::emit \
      error \
      post_deploy_smoke.frontend_session_authenticated.failed \
      "Authenticated frontend session smoke redirected outside the expected route" \
      smoke_service="$service" \
      check_label="$label" \
      target_url="$session_url" \
      effective_url="$effective_url" \
  ...
  if ! response_header_includes "$headers_path" "cache-control" "no-store"; then
    ...
    json_log::emit \
      error \
      post_deploy_smoke.frontend_session_authenticated.failed \
      "Authenticated frontend session smoke is missing no-store headers" \
      smoke_service="$service" \
      check_label="$label" \
      target_url="$session_url" \
      status=failed \
  ...
  if ! validate_authenticated_session_body "$service" "$body_path"; then
    rm -rf "$temp_dir"
    json_log::emit \
      error \
      post_deploy_smoke.frontend_session_authenticated.failed \
      "Authenticated frontend session smoke returned an invalid session body" \
      smoke_service="$service" \
      check_label="$label" \
      target_url="$session_url" \
      status=failed \
      reason=invalid_session_body
    echo "FAIL"
    record_fail
    return
  fi
  ...
  json_log::emit \
    info \
    post_deploy_smoke.frontend_session_authenticated.completed \
    "Authenticated frontend session smoke passed" \
    smoke_service="$service" \
    check_label="$label" \
    target_url="$session_url" \
    effective_url="$effective_url" \
    status=completed
  echo "OK (200 authenticated session)"
  record_pass
}
      ...
      case "$service" in
        webapp | admin)
          ...
          run_frontend_authenticated_session_check \
            "$service" \
            "$base_url" \
            "$(resolve_frontend_session_cookie_file "$service")" \
            "GET ${service} /auth/session authenticated"
          run_frontend_session_check \
            "$service" \
            "$base_url" \
            "$base_url" \
            ...
            "401" \
            "unauthorized" \
            "GET ${service} /auth/session unauthenticated"
          run_frontend_session_check \
            "$service" \
            "$base_url" \
            "https://evil.example" \
