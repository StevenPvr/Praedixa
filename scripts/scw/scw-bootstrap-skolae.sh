...
ensure_domain_binding_and_dns() {
  ...
  if [ -z "$existing_domain" ]; then
    ...
    scw container domain create hostname="$SKOLAE_HOSTNAME" container-id="$container_id" region="$REGION" >/dev/null
  else
  ...
  for _ in $(seq 1 "$VERIFY_ATTEMPTS"); do
    ...
    binding_json="$(scw container domain list region="$REGION" -o json | jq -c --arg h "$SKOLAE_HOSTNAME" '.[] | select(.hostname==$h)' | head -n1)"
    ...
    if [ "$bound_container_id" = "$container_id" ]; then
      if [ -z "$binding_status" ] || [ "$binding_status" = "ready" ] || [ "$binding_status" = "pending" ]; then
        echo "[verified] container domain $SKOLAE_HOSTNAME bound to $CONTAINER_NAME"
        break
      fi
      if [ "$binding_status" = "error" ]; then
        echo "Container domain binding for $SKOLAE_HOSTNAME is in error state" >&2
        exit 1
      fi
    fi

    if [ "$_" -eq "$VERIFY_ATTEMPTS" ]; then
      echo "Container domain binding for $SKOLAE_HOSTNAME was not observable after verification window" >&2
      exit 1
    fi
  ...
  for _ in $(seq 1 "$VERIFY_ATTEMPTS"); do
    ...
    public_cname_target="$(resolve_public_cname_target "$SKOLAE_HOSTNAME" | tr -d '[:space:]')"
    if [ -n "$public_cname_target" ] && [ "$(normalize_dns_target "$public_cname_target")" = "$domain_name" ]; then
      echo "[verified] public DNS CNAME ${SKOLAE_HOSTNAME} -> $domain_name"
      return
    fi

    if [ "$_" -eq "$VERIFY_ATTEMPTS" ]; then
      echo "Public DNS for $SKOLAE_HOSTNAME does not resolve to expected CNAME target ${domain_name}" >&2
