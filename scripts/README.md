...
`gate:architecture:frontend` rejoue `dependency-cruiser` sur `app-landing`, `app-webapp`, `app-admin` et `packages/`, puis `knip` et `architecture:ts-guardrails:frontend` pour rendre les dependances front/shared et la focalisation des fichiers auditables en local comme en CI sans se faire bloquer par la dette historique hors perimetre.
`gate:architecture:api` applique un graphe de dependances dedie aux runtimes `app-api-ts` et `app-connectors`, puis `architecture:ts-guardrails:api`, afin d'interdire les imports directs vers les frontends, les couplages runtime <-> runtime non voulus et toute nouvelle derive de taille sur le socle TS cote API.
`architecture:ts-guardrails` compare la taille des fichiers et fonctions TS/JS a une baseline versionnee dans `scripts/ts-guardrail-baseline.json`; il bloque uniquement les nouvelles violations ou les regressions, pas la dette historique deja capturee. Les variantes `:frontend` et `:api` reutilisent cette meme baseline mais ne scannent que leur surface cible.
...
`validate-build-ready-status.mjs` verifie le verdict machine-readable `docs/governance/build-ready-status.json`: schema, clusters, blockers, references de preuve et invariants `Go/No-Go`.
`generate-build-ready-report.mjs` attache ensuite ce verdict a un SHA reel dans `.git/gate-reports/build-ready-<sha>.json`, pour que `CI - Autorite` publie une preuve relisible et stable du statut structurel du repo.
`validate-github-workflow-pnpm-order.mjs` bloque le piege GitHub Actions ou `actions/setup-node` est execute avec `cache: pnpm` avant `pnpm/action-setup`; sans cet ordre, le runner peut echouer tres tot sur `Unable to locate executable file: pnpm`.
Les jobs GitHub qui construisent `app-admin` doivent aussi reconstruire `@praedixa/api-hooks` au meme titre que `@praedixa/shared-types` et `@praedixa/ui`, car `app-admin` importe les exports `dist/*` de ce package et un runner CI propre n'heritera jamais d'un `dist/` local stale.
`validate-turbo-env-coverage.mjs` bloque tout drift entre `turbo.json` et les vrais fichiers/env consommes par `scripts/lib/local-env.sh`, afin que les caches Turbo restent invalides quand la configuration runtime change reellement.
...
`github/verify-main-branch-protection.sh` verifie via GitHub API que `main` exige bien `Autorite - Required`, `strict = true`, `1` review obligatoire, `enforce_admins = true`, la resolution de conversation, l'historique lineaire et l'interdiction des force-push/suppressions.
`github/verify-main-required-check.sh` complete cette preuve en lisant l'API Checks GitHub pour verifier que le HEAD de `main` expose bien un `Autorite - Required` termine en succes, meme quand l'API statuses classique n'est pas assez lisible.
...
`ci/run-authoritative-ci.sh` est le point d'entree CI canonique: il enchaine delta securite, gate exhaustif signe, reverification du rapport, contrat runtime secrets + contrat runtime env genere, validation `build-ready`, validation Turbo/env, tests de contrat release, puis generation du rapport `build-ready-<sha>.json` pour que GitHub Actions puisse devenir l'arbitre final sans dupliquer la logique de fond dans chaque workflow.
Dans `CI - Autorite`, ces gates shell sont desormais lances via `uv run --with pyyaml` pour garantir `PyYAML` meme sur un runner nu; cela evite que `validate-security-exceptions.py` ou les snippets YAML du gate cassent avant le vrai signal de fond.
...
Comme `tsc` des apps Next peut regenerer `app-*/next-env.d.ts`, ce gate restaure aussi ces fichiers derives avant de sortir pour qu'un `pre-push` vert ne laisse jamais le worktree sale.
...
`scw-release-deploy.sh` doit utiliser la reference signee `registry-image@sha256:...` du manifest comme source de verite; si l'API Scaleway Container refuse cette syntaxe, le script doit echouer explicitement et ne pas retomber sur un tag mutable.
...
Le chemin nominal de prod passe maintenant par le workflow GitHub Actions `Release - Platform`; les scripts `release:*` et `scw-*` restent les primitives versionnees appelees par la CI, ainsi que les outils de preflight/break-glass.
`scw-iac-validate.sh` valide le module racine `infra/opentofu/` puis les states `environments/staging` et `environments/prod`. `scw-iac-output.sh` et `lib/scw-topology.sh` preferent ensuite les outputs state-backed quand un backend d'environnement existe, avec repli lecture seule vers `platform-topology.json` uniquement pour le bootstrap local et les validations hors state distant.
