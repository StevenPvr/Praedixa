...
`gate-prepush-deep.sh` resynchronise d'abord `app-api/.venv` avec `uv sync --all-groups --locked` avant `pip-audit`, pour auditer le graphe verrouille du sous-projet et non un environnement local stale charge d'anciens outils.
