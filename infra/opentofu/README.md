...
Le repo ne doit plus dependre uniquement de scripts `scw-*` disperses comme source de verite infra. Le point d'entree durable devient le catalogue [platform-topology.json](./platform-topology.json), materialise ensuite dans des states par environnement (`environments/staging`, `environments/prod`) et consomme a la fois :
