...
output "service_contracts" {
  ...
  description = "Contrats de service staging consommes par les wrappers et la CI."
}

output "platform_targets" {
  value       = module.platform.platform_targets[local.environment]
  description = "Cibles de release staging derivees du catalogue declaratif."
}

output "platform_topology" {
  value       = module.platform.platform_topology
  description = "Topologie canonique conservee dans le state staging."
}
