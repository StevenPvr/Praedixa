...
export function canAccessOrganization(
  ...
  return (
    principal.allowedOrgs.includes("global:all-orgs") ||
    principal.allowedOrgs.includes(organizationId)
  );
}
    ...
    (async () => {
      ...
      const logResult = (
        ...
        const correlationHeaders = buildResponseCorrelationHeaders({
          ...
          connectorRunId:
            requestLogState?.connectorRunId ??
            requestCorrelation.connectorRunId,
        });
        if (requestLogState != null) {
      ...
      if (matched.route.authRequired) {
        ...
        if (
          !hasRequiredCapabilities(
            principal,
            matched.route.requiredCapabilities,
          )
        ) {
