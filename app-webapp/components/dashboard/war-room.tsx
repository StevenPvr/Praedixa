...
export function WarRoomDashboard() {
  ...
  const hasError =
    summaryQuery.error || alertsQuery.error || qualityQuery.error;
    ...
    <div className="space-y-8 pb-8">
      <section className="space-y-2">
        ...
        <p className="text-sm text-ink-secondary">
          Vue synthese des risques, de la qualite data et des prochaines
          actions.
        </p>
      </section>
              ...
              {topAlerts.map((alert) => (
                ...
                >
                  ...
                  <p className="mt-1 text-xs text-ink-secondary">
                    ...
                    {formatPercent(alert.pRupture)} · Impact:{" "}
                    {formatCurrency(alert.impactEur ?? null)}
                  </p>
                </li>
              ))}
            </ul>
            ...
            <dl className="mt-4 space-y-2 text-sm text-ink-secondary">
              <div className="flex items-center justify-between">
                ...
                <dd className="font-medium text-ink">
                  {qualityQuery.data?.totalRecords ?? "--"}
                </dd>
              </div>
              <div className="flex items-center justify-between">
                <dt>Sites couverts</dt>
                <dd className="font-medium text-ink">
                  {qualityQuery.data?.sites ?? "--"}
                </dd>
              </div>
              <div className="flex items-center justify-between">
