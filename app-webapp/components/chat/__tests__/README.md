# Tests

## Rôle

Ce dossier regroupe les tests associés au dossier parent `app-webapp/components/chat`. Il documente et vérifie le comportement attendu de cette zone du code.

## Contenu immédiat

Sous-dossiers :

- Aucun élément versionné direct.

Fichiers :

- `conversation-list.test.tsx`
- `message-input.test.tsx`
- `message-thread.test.tsx`

## Intégration

Ce dossier est consommé par l'application `app-webapp` et s'insère dans son flux runtime, build ou test.
