"""Mapping helpers for Olo pull -> connector raw events."""
...
_DEFAULT_RECORD_ID_FIELDS = {
    "Orders": ("id", "orderId", "externalOrderId", "guid"),
    "Stores": ("id", "storeId", "externalId"),
    "Products": ("id", "productId", "sku", "externalId"),
    "Promotions": ("id", "promotionId", "externalId"),
}
_DEFAULT_UPDATED_AT_FIELDS = {
    "Orders": ("updatedAt", "modifiedAt", "closedAt", "createdAt"),
    "Stores": ("updatedAt", "modifiedAt", "createdAt"),
    "Products": ("updatedAt", "modifiedAt", "createdAt"),
    "Promotions": ("updatedAt", "modifiedAt", "createdAt"),
}


@dataclass(frozen=True)
class OloObjectRequest:
    source_object: str
    path: str
    params: dict[str, str]
    items_path: str | None
...
def build_olo_requests(
    connection: dict[str, Any],
    source_objects: tuple[str, ...],
    claimed_run: RuntimeClaimedSyncRun,
) -> tuple[OloObjectRequest, ...]:
    config = connection.get("config", {})
    if not isinstance(config, dict):
        raise TypeError("Olo connection config must be an object")

    raw_endpoints = config.get("oloEndpoints")
    if not isinstance(raw_endpoints, dict):
        raise TypeError("Olo config.oloEndpoints must be configured")

    requests: list[OloObjectRequest] = []
    for source_object in source_objects:
        if source_object not in _SUPPORTED_SOURCE_OBJECTS:
            raise ValueError(f"Unsupported Olo source object: {source_object}")
        raw_endpoint = raw_endpoints.get(source_object)
        if not isinstance(raw_endpoint, dict):
            raise TypeError(
                f"Olo config.oloEndpoints.{source_object} must be configured"
            )
...
def map_olo_record_to_event(
    ...
    *,
    ...
    if record_id is None:
        raise ValueError(f"Olo {source_object} record is missing an identifier")
    ...
    event_id = f"olo:{source_object}:{record_id}:{source_updated_at or 'na'}"
...
def _build_request_params(
    ...
    if not isinstance(page_size, int) or page_size < 1 or page_size > _MAX_PAGE_SIZE:
        raise ValueError(
            f"Olo endpoint pageSize must be an integer between 1 and {_MAX_PAGE_SIZE}"
        )
    page_size_param = (
...
def _normalize_string_map(value: Any, field: str) -> dict[str, str]:
    ...
    if not isinstance(value, dict):
        raise TypeError(f"Olo endpoint {field} must be an object")

    normalized: dict[str, str] = {}
    for raw_key, raw_value in value.items():
        key = str(raw_key).strip()
        if not key:
            raise ValueError(f"Olo endpoint {field} contains an empty key")
        ...
        if text_value is None:
            raise ValueError(
                f"Olo endpoint {field}.{key} must be a string, number or boolean"
            )
...
def _required_string(value: Any, field: str) -> str:
    ...
    if text is None:
        raise ValueError(f"Olo endpoint {field} must be configured")
    if not text.startswith("/"):
        raise ValueError(f"Olo endpoint {field} must start with '/'")
    return text
