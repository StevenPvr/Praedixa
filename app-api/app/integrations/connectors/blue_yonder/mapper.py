"""Mapping helpers for Blue Yonder pull -> connector raw events."""
...
_DEFAULT_RECORD_ID_FIELDS = {
    "DemandPlan": ("planId", "id", "externalPlanId"),
    "LaborPlan": ("laborPlanId", "id", "externalLaborPlanId"),
    "Store": ("storeId", "id", "externalStoreId"),
    "SKU": ("sku", "itemId", "id"),
}
_DEFAULT_UPDATED_AT_FIELDS = {
    "DemandPlan": ("updatedAt", "modifiedAt", "effectiveAt"),
    "LaborPlan": ("updatedAt", "modifiedAt", "effectiveAt"),
    "Store": ("updatedAt", "modifiedAt", "createdAt"),
    "SKU": ("updatedAt", "modifiedAt", "createdAt"),
}


@dataclass(frozen=True)
class BlueYonderObjectRequest:
    source_object: str
    path: str
    params: dict[str, str]
    items_path: str | None
...
def build_blue_yonder_requests(
...
def map_blue_yonder_record_to_event(
    source_object: str,
    record: dict[str, Any],
    *,
    record_id_field: str | None,
    ...
    if record_id is None:
        raise ValueError(
            f"Blue Yonder {source_object} record is missing an identifier"
        )
    ...
    event_id = (
        f"blue_yonder:{source_object}:{record_id}:{source_updated_at or 'na'}"
    )
...
def _build_request_params(
    ...
    if not isinstance(page_size, int) or page_size < 1 or page_size > _MAX_PAGE_SIZE:
        raise ValueError(
            "Blue Yonder endpoint pageSize must be an integer between 1 and "
            f"{_MAX_PAGE_SIZE}"
        )
    page_size_param = (
...
def _required_string(value: Any, field: str) -> str:
    ...
    if text is None:
        raise ValueError(f"Blue Yonder endpoint {field} must be configured")
    if not text.startswith("/"):
        raise ValueError(f"Blue Yonder endpoint {field} must start with '/'")
    return text
