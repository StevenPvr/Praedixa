"""Mapping helpers for CDK pull -> connector raw events."""
...
_DEFAULT_RECORD_ID_FIELDS = {
    "ServiceOrders": ("serviceOrderId", "repairOrderNumber", "roNumber", "id"),
    "ROLines": ("roLineId", "lineId", "opCode", "id"),
    "Vehicle": ("vehicleId", "vin", "stockNumber", "id"),
    "Technician": ("technicianId", "employeeId", "id"),
}
_DEFAULT_UPDATED_AT_FIELDS = {
    "ServiceOrders": ("updatedAt", "modifiedAt", "closedAt", "openedAt"),
    "ROLines": ("updatedAt", "modifiedAt", "closedAt"),
    "Vehicle": ("updatedAt", "modifiedAt", "inventoryDate"),
    "Technician": ("updatedAt", "modifiedAt", "activeSince"),
}


@dataclass(frozen=True)
class CdkObjectRequest:
    source_object: str
    path: str
    params: dict[str, str]
    items_path: str | None
...
def build_cdk_requests(
    connection: dict[str, Any],
    source_objects: tuple[str, ...],
    claimed_run: RuntimeClaimedSyncRun,
) -> tuple[CdkObjectRequest, ...]:
    config = connection.get("config", {})
    if not isinstance(config, dict):
        raise TypeError("CDK connection config must be an object")

    raw_endpoints = config.get("cdkEndpoints")
    if not isinstance(raw_endpoints, dict):
        raise TypeError("CDK config.cdkEndpoints must be configured")

    requests: list[CdkObjectRequest] = []
    for source_object in source_objects:
        if source_object not in _SUPPORTED_SOURCE_OBJECTS:
            raise ValueError(f"Unsupported CDK source object: {source_object}")
        raw_endpoint = raw_endpoints.get(source_object)
        if not isinstance(raw_endpoint, dict):
            raise TypeError(
                f"CDK config.cdkEndpoints.{source_object} must be configured"
            )
...
def map_cdk_record_to_event(
    ...
    *,
    ...
    if record_id is None:
        raise ValueError(f"CDK {source_object} record is missing an identifier")
    ...
    event_id = f"cdk:{source_object}:{record_id}:{source_updated_at or 'na'}"
...
def _build_request_params(
    ...
    if not isinstance(page_size, int) or page_size < 1 or page_size > _MAX_PAGE_SIZE:
        raise ValueError(
            f"CDK endpoint pageSize must be an integer between 1 and {_MAX_PAGE_SIZE}"
        )
    page_size_param = (
...
def _normalize_string_map(value: Any, field: str) -> dict[str, str]:
    ...
    if not isinstance(value, dict):
        raise TypeError(f"CDK endpoint {field} must be an object")

    normalized: dict[str, str] = {}
    for raw_key, raw_value in value.items():
        key = str(raw_key).strip()
        if not key:
            raise ValueError(f"CDK endpoint {field} contains an empty key")
        ...
        if text_value is None:
            raise ValueError(
                f"CDK endpoint {field}.{key} must be a string, number or boolean"
            )
...
def _required_string(value: Any, field: str) -> str:
    ...
    if text is None:
        raise ValueError(f"CDK endpoint {field} must be configured")
    if not text.startswith("/"):
        raise ValueError(f"CDK endpoint {field} must start with '/'")
    return text
