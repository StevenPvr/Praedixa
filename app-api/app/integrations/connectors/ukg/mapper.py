...
def build_ukg_requests(
    ...
    if not isinstance(config, dict):
        raise TypeError("UKG connection config must be an object")

    raw_endpoints = config.get("ukgEndpoints")
    if not isinstance(raw_endpoints, dict):
        raise TypeError("UKG config.ukgEndpoints must be configured")
    ...
    for source_object in source_objects:
        ...
        if not isinstance(raw_endpoint, dict):
            raise TypeError(
                f"UKG config.ukgEndpoints.{source_object} must be configured"
            )
...
def _normalize_string_map(value: Any, field: str) -> dict[str, str]:
    ...
    if not isinstance(value, dict):
        raise TypeError(f"UKG endpoint {field} must be an object")

    normalized: dict[str, str] = {}
    for raw_key, raw_value in value.items():
        key = str(raw_key).strip()
        if not key:
            raise ValueError(f"UKG endpoint {field} contains an empty key")
        ...
        if text_value is None:
            raise ValueError(
                f"UKG endpoint {field}.{key} must be a string, number or boolean"
            )
