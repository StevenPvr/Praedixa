"""Mapping helpers for Oracle TM pull -> connector raw events."""
...
_MAX_PAGE_SIZE = 1000
_DEFAULT_PAGE_SIZE_PARAM = "limit"
_DEFAULT_CURSOR_PARAM = "offset"
_DEFAULT_UPDATED_AFTER_PARAM = "updatedAfter"
_DEFAULT_UPDATED_BEFORE_PARAM = "updatedBefore"
_DEFAULT_RECORD_ID_FIELDS = {
    "Shipment": ("shipmentGid", "shipmentId", "gid", "id"),
    "OrderRelease": ("orderReleaseGid", "orderReleaseId", "gid", "id"),
    "Route": ("routeGid", "itineraryGid", "gid", "id"),
    "Stop": ("stopId", "stopGid", "stopSequence", "id"),
}
_DEFAULT_UPDATED_AT_FIELDS = {
    "Shipment": ("lastUpdateDate", "updatedAt", "insertDate"),
    "OrderRelease": ("lastUpdateDate", "updatedAt", "insertDate"),
    "Route": ("lastUpdateDate", "updatedAt", "plannedStartTime"),
    "Stop": ("lastUpdateDate", "eventTime", "updatedAt", "arrivalTime"),
}


@dataclass(frozen=True)
class OracleTmObjectRequest:
    source_object: str
    path: str
    params: dict[str, str]
    items_path: str | None
...
def build_oracle_tm_requests(
    connection: dict[str, Any],
    source_objects: tuple[str, ...],
    claimed_run: RuntimeClaimedSyncRun,
) -> tuple[OracleTmObjectRequest, ...]:
    config = connection.get("config", {})
    if not isinstance(config, dict):
        raise TypeError("Oracle TM connection config must be an object")

    raw_endpoints = config.get("oracleTmEndpoints")
    if not isinstance(raw_endpoints, dict):
        raise TypeError("Oracle TM config.oracleTmEndpoints must be configured")

    requests: list[OracleTmObjectRequest] = []
    for source_object in source_objects:
        if source_object not in _SUPPORTED_SOURCE_OBJECTS:
            raise ValueError(f"Unsupported Oracle TM source object: {source_object}")
        raw_endpoint = raw_endpoints.get(source_object)
        if not isinstance(raw_endpoint, dict):
            raise TypeError(
                f"Oracle TM config.oracleTmEndpoints.{source_object} must be configured"
            )

        requests.append(
            OracleTmObjectRequest(
                source_object=source_object,
                path=_required_string(
                    raw_endpoint.get("path"),
                ...
                next_cursor_path=_optional_string(
                    raw_endpoint.get("nextCursorPath")
                ),
                cursor_param=(
...
def map_oracle_tm_record_to_event(
    source_object: str,
    record: dict[str, Any],
    *,
    record_id_field: str | None,
    ...
    if record_id is None:
        raise ValueError(f"Oracle TM {source_object} record is missing an identifier")
    ...
    event_id = f"oracle_tm:{source_object}:{record_id}:{source_updated_at or 'na'}"
...
def _build_request_params(
    ...
    if not isinstance(page_size, int) or page_size < 1 or page_size > _MAX_PAGE_SIZE:
        raise ValueError(
            "Oracle TM endpoint pageSize must be an integer between 1 and "
            f"{_MAX_PAGE_SIZE}"
        )
    page_size_param = (
...
def _normalize_string_map(value: Any, field: str) -> dict[str, str]:
    ...
    if not isinstance(value, dict):
        raise TypeError(f"Oracle TM endpoint {field} must be an object")

    normalized: dict[str, str] = {}
    for raw_key, raw_value in value.items():
        key = str(raw_key).strip()
        if not key:
            raise ValueError(f"Oracle TM endpoint {field} contains an empty key")
        ...
        if text_value is None:
            raise ValueError(
                f"Oracle TM endpoint {field}.{key} must be a string, number or boolean"
            )
...
def _required_string(value: Any, field: str) -> str:
    ...
    if text is None:
        raise ValueError(f"Oracle TM endpoint {field} must be configured")
    if not text.startswith("/"):
        raise ValueError(f"Oracle TM endpoint {field} must start with '/'")
    return text
