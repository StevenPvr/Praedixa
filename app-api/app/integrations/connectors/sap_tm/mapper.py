"""Mapping helpers for SAP TM pull -> connector raw events."""
...
_MAX_PAGE_SIZE = 1000
_DEFAULT_PAGE_SIZE_PARAM = "$top"
_DEFAULT_CURSOR_PARAM = "$skiptoken"
_DEFAULT_UPDATED_AFTER_PARAM = "$filter"
_DEFAULT_UPDATED_BEFORE_PARAM = "$filterEnd"
_DEFAULT_RECORD_ID_FIELDS = {
    "FreightOrder": ("FreightOrder", "TransportationOrderUUID", "id"),
    "FreightUnit": ("FreightUnit", "TransportationUnitUUID", "id"),
    "Resource": ("Resource", "ResourceUUID", "id"),
    "Stop": ("Stop", "StopUUID", "id"),
}
_DEFAULT_UPDATED_AT_FIELDS = {
    "FreightOrder": ("ChangedOn", "LastChangeDateTime", "updatedAt"),
    "FreightUnit": ("ChangedOn", "LastChangeDateTime", "updatedAt"),
    "Resource": ("ChangedOn", "LastChangeDateTime", "updatedAt"),
    "Stop": ("ChangedOn", "ActualDateTime", "updatedAt"),
}


@dataclass(frozen=True)
class SapTmObjectRequest:
    source_object: str
    path: str
    params: dict[str, str]
    items_path: str | None
...
def build_sap_tm_requests(
    connection: dict[str, Any],
    source_objects: tuple[str, ...],
    claimed_run: RuntimeClaimedSyncRun,
) -> tuple[SapTmObjectRequest, ...]:
    config = connection.get("config", {})
    if not isinstance(config, dict):
        raise TypeError("SAP TM connection config must be an object")

    raw_endpoints = config.get("sapTmEndpoints")
    if not isinstance(raw_endpoints, dict):
        raise TypeError("SAP TM config.sapTmEndpoints must be configured")

    requests: list[SapTmObjectRequest] = []
    for source_object in source_objects:
        if source_object not in _SUPPORTED_SOURCE_OBJECTS:
            raise ValueError(f"Unsupported SAP TM source object: {source_object}")
        raw_endpoint = raw_endpoints.get(source_object)
        if not isinstance(raw_endpoint, dict):
            raise TypeError(
                f"SAP TM config.sapTmEndpoints.{source_object} must be configured"
            )
...
def map_sap_tm_record_to_event(
    source_object: str,
    record: dict[str, Any],
    *,
    record_id_field: str | None,
    ...
    if record_id is None:
        raise ValueError(f"SAP TM {source_object} record is missing an identifier")
    ...
    event_id = f"sap_tm:{source_object}:{record_id}:{source_updated_at or 'na'}"
...
def _build_request_params(
    ...
    if not isinstance(page_size, int) or page_size < 1 or page_size > _MAX_PAGE_SIZE:
        raise ValueError(
            "SAP TM endpoint pageSize must be an integer between 1 and "
            f"{_MAX_PAGE_SIZE}"
        )
    page_size_param = (
...
def _normalize_string_map(value: Any, field: str) -> dict[str, str]:
    ...
    if not isinstance(value, dict):
        raise TypeError(f"SAP TM endpoint {field} must be an object")

    normalized: dict[str, str] = {}
    for raw_key, raw_value in value.items():
        key = str(raw_key).strip()
        if not key:
            raise ValueError(f"SAP TM endpoint {field} contains an empty key")
        ...
        if text_value is None:
            raise ValueError(
                f"SAP TM endpoint {field}.{key} must be a string, number or boolean"
            )
...
def _required_string(value: Any, field: str) -> str:
    ...
    if text is None:
        raise ValueError(f"SAP TM endpoint {field} must be configured")
    if not text.startswith("/"):
        raise ValueError(f"SAP TM endpoint {field} must start with '/'")
    return text
