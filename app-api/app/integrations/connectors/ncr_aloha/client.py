"""Minimal NCR Aloha REST client used by the provider sync worker."""
...
class NcrAlohaApiClient:
    """Thin async client for NCR Aloha REST endpoints with cursor pagination."""
...
def _resolve_items(payload: Any, items_path: str | None) -> Any:
    ...
    raise TypeError("NCR Aloha response does not contain a list of records")


def _normalize_records(raw_items: Any) -> list[dict[str, Any]]:
    if not isinstance(raw_items, Sequence) or isinstance(raw_items, (str, bytes)):
        raise TypeError("NCR Aloha response records must be a list")

    records: list[dict[str, Any]] = []
    for raw_item in raw_items:
        if not isinstance(raw_item, dict):
            raise TypeError("NCR Aloha record must be an object")
