"""Mapping helpers for NCR Aloha pull -> connector raw events."""
...
_DEFAULT_RECORD_ID_FIELDS = {
    "Check": ("checkId", "id", "ticketId"),
    "Item": ("itemId", "id", "lineId"),
    "Labor": ("laborId", "id", "employeeShiftId"),
    "Inventory": ("inventoryId", "id", "itemSku"),
}
_DEFAULT_UPDATED_AT_FIELDS = {
    "Check": ("updatedAt", "modifiedAt", "closedAt", "openAt"),
    "Item": ("updatedAt", "modifiedAt", "businessDate"),
    "Labor": ("updatedAt", "modifiedAt", "clockOutAt", "clockInAt"),
    "Inventory": ("updatedAt", "modifiedAt", "snapshotAt"),
}


@dataclass(frozen=True)
class NcrAlohaObjectRequest:
    source_object: str
    path: str
    params: dict[str, str]
    items_path: str | None
...
def build_ncr_aloha_requests(
    connection: dict[str, Any],
    source_objects: tuple[str, ...],
    claimed_run: RuntimeClaimedSyncRun,
) -> tuple[NcrAlohaObjectRequest, ...]:
    config = connection.get("config", {})
    if not isinstance(config, dict):
        raise TypeError("NCR Aloha connection config must be an object")

    raw_endpoints = config.get("alohaEndpoints")
    if not isinstance(raw_endpoints, dict):
        raise TypeError("NCR Aloha config.alohaEndpoints must be configured")

    requests: list[NcrAlohaObjectRequest] = []
    for source_object in source_objects:
        if source_object not in _SUPPORTED_SOURCE_OBJECTS:
            raise ValueError(
                f"Unsupported NCR Aloha source object: {source_object}"
            )
        raw_endpoint = raw_endpoints.get(source_object)
        if not isinstance(raw_endpoint, dict):
            raise TypeError(
                f"NCR Aloha config.alohaEndpoints.{source_object} must be configured"
            )

        path = _required_string(raw_endpoint.get("path"), f"{source_object}.path")
        params = _build_request_params(raw_endpoint, claimed_run)
        requests.append(
            NcrAlohaObjectRequest(
...
def map_ncr_aloha_record_to_event(
    source_object: str,
    record: dict[str, Any],
    *,
    record_id_field: str | None,
    ...
    if record_id is None:
        raise ValueError(
            f"NCR Aloha {source_object} record is missing an identifier"
        )
    ...
    event_id = f"ncr_aloha:{source_object}:{record_id}:{source_updated_at or 'na'}"
...
def _build_request_params(
    ...
    if not isinstance(page_size, int) or page_size < 1 or page_size > _MAX_PAGE_SIZE:
        raise ValueError(
            "NCR Aloha endpoint pageSize must be an integer between 1 and "
            f"{_MAX_PAGE_SIZE}"
        )
    page_size_param = (
...
def _normalize_string_map(value: Any, field: str) -> dict[str, str]:
    ...
    if not isinstance(value, dict):
        raise TypeError(f"NCR Aloha endpoint {field} must be an object")

    normalized: dict[str, str] = {}
    for raw_key, raw_value in value.items():
        key = str(raw_key).strip()
        if not key:
            raise ValueError(f"NCR Aloha endpoint {field} contains an empty key")
        ...
        if text_value is None:
            raise ValueError(
                f"NCR Aloha endpoint {field}.{key} must be a string, number or boolean"
            )
...
def _required_string(value: Any, field: str) -> str:
    ...
    if text is None:
        raise ValueError(f"NCR Aloha endpoint {field} must be configured")
    if not text.startswith("/"):
        raise ValueError(f"NCR Aloha endpoint {field} must start with '/'")
    return text
