"""Mapping helpers for Fourth pull -> connector raw events."""
...
_DEFAULT_RECORD_ID_FIELDS = {
    "Employees": ("employeeId", "personId", "id"),
    "Roster": ("shiftId", "rosterId", "id"),
    "Timeclock": ("timeRecordId", "clockingId", "id"),
    "LaborForecast": ("forecastId", "laborCostId", "id"),
}
_DEFAULT_UPDATED_AT_FIELDS = {
    "Employees": ("updatedAt", "modifiedAt", "lastChangedDateTime"),
    "Roster": ("updatedAt", "modifiedAt", "shiftStart"),
    "Timeclock": ("updatedAt", "modifiedAt", "clockInDateTime"),
    "LaborForecast": ("updatedAt", "modifiedAt", "businessDate"),
}


@dataclass(frozen=True)
class FourthObjectRequest:
    source_object: str
    path: str
    params: dict[str, str]
    items_path: str | None
...
def build_fourth_requests(
    connection: dict[str, Any],
    source_objects: tuple[str, ...],
    claimed_run: RuntimeClaimedSyncRun,
) -> tuple[FourthObjectRequest, ...]:
    config = connection.get("config", {})
    if not isinstance(config, dict):
        raise TypeError("Fourth connection config must be an object")

    raw_endpoints = config.get("fourthEndpoints")
    if not isinstance(raw_endpoints, dict):
        raise TypeError("Fourth config.fourthEndpoints must be configured")

    requests: list[FourthObjectRequest] = []
    for source_object in source_objects:
        if source_object not in _SUPPORTED_SOURCE_OBJECTS:
            raise ValueError(f"Unsupported Fourth source object: {source_object}")
        raw_endpoint = raw_endpoints.get(source_object)
        if not isinstance(raw_endpoint, dict):
            raise TypeError(
                f"Fourth config.fourthEndpoints.{source_object} must be configured"
            )
...
def map_fourth_record_to_event(
    source_object: str,
    record: dict[str, Any],
    *,
    record_id_field: str | None,
    ...
    if record_id is None:
        raise ValueError(f"Fourth {source_object} record is missing an identifier")
    ...
    event_id = f"fourth:{source_object}:{record_id}:{source_updated_at or 'na'}"
...
def _build_request_params(
    ...
    if not isinstance(page_size, int) or page_size < 1 or page_size > _MAX_PAGE_SIZE:
        raise ValueError(
            "Fourth endpoint pageSize must be an integer between 1 and "
            f"{_MAX_PAGE_SIZE}"
        )
    page_size_param = (
...
def _normalize_string_map(value: Any, field: str) -> dict[str, str]:
    ...
    if not isinstance(value, dict):
        raise TypeError(f"Fourth endpoint {field} must be an object")

    normalized: dict[str, str] = {}
    for raw_key, raw_value in value.items():
        key = str(raw_key).strip()
        if not key:
            raise ValueError(f"Fourth endpoint {field} contains an empty key")
        ...
        if text_value is None:
            raise ValueError(
                f"Fourth endpoint {field}.{key} must be a string, number or boolean"
            )
...
def _required_string(value: Any, field: str) -> str:
    ...
    if text is None:
        raise ValueError(f"Fourth endpoint {field} must be configured")
    if not text.startswith("/"):
        raise ValueError(f"Fourth endpoint {field} must start with '/'")
    return text
