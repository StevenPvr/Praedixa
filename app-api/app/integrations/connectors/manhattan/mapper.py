"""Mapping helpers for Manhattan pull -> connector raw events."""
...
_DEFAULT_RECORD_ID_FIELDS = {
    "Wave": ("waveId", "id", "externalWaveId"),
    "Task": ("taskId", "id", "externalTaskId"),
    "Inventory": ("inventoryId", "id", "sku"),
    "Shipment": ("shipmentId", "id", "externalShipmentId"),
}
_DEFAULT_UPDATED_AT_FIELDS = {
    "Wave": ("updatedAt", "modifiedAt", "startTime"),
    "Task": ("updatedAt", "modifiedAt", "completedAt"),
    "Inventory": ("updatedAt", "modifiedAt", "snapshotAt"),
    "Shipment": ("updatedAt", "modifiedAt", "eta"),
}


@dataclass(frozen=True)
class ManhattanObjectRequest:
    source_object: str
    path: str
    params: dict[str, str]
    items_path: str | None
...
def map_manhattan_record_to_event(
    source_object: str,
    record: dict[str, Any],
    *,
    record_id_field: str | None,
    ...
    if record_id is None:
        raise ValueError(
            f"Manhattan {source_object} record is missing an identifier"
        )
    ...
    event_id = f"manhattan:{source_object}:{record_id}:{source_updated_at or 'na'}"
...
def _build_request_params(
    ...
    if not isinstance(page_size, int) or page_size < 1 or page_size > _MAX_PAGE_SIZE:
        raise ValueError(
            "Manhattan endpoint pageSize must be an integer between 1 and "
            f"{_MAX_PAGE_SIZE}"
        )
    page_size_param = (
...
def _required_string(value: Any, field: str) -> str:
    ...
    if text is None:
        raise ValueError(f"Manhattan endpoint {field} must be configured")
    if not text.startswith("/"):
        raise ValueError(f"Manhattan endpoint {field} must start with '/'")
    return text
