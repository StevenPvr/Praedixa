...
class TestGenerateMockForecasts:
    ...
    @pytest.mark.asyncio
    async def test_low_bin_calibration_bucket_is_used(self) -> None:
        tenant = _make_tenant()
        records = [
            _make_canonical_record(
                site_id="site-paris",
        ...
        with patch.object(random.Random, "gauss", return_value=-1.0):
            count = await generate_mock_forecasts(session, tenant)

        assert count > 0
        added_alerts = [call.args[0] for call in session.add.call_args_list]
        assert any(a.calibration_bucket == "low_bin" for a in added_alerts)
