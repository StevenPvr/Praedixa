...
class TestHelpers:
    def test_horizon_to_days_j7_and_default(self) -> None:
        assert _horizon_to_days("j7") == 7
        assert _horizon_to_days("unknown") == 14

    def test_clamp01_bounds(self) -> None:
        assert _clamp01(Decimal("-0.1")) == Decimal("0")
        assert _clamp01(Decimal("1.1")) == Decimal("1")
...
class TestSelectRecommendation:
    ...
    def test_explicit_policy_without_compliant_returns_none(self) -> None:
        strict_a = _make_scenario_option(
            cout_total_eur=Decimal("100"),
            service_attendu_pct=Decimal("0.9999"),
            policy_compliance=False,
        )
        strict_b = _make_scenario_option(
            cout_total_eur=Decimal("200"),
            service_attendu_pct=Decimal("1.0000"),
            policy_compliance=False,
        )
        assert select_recommendation([strict_a, strict_b]) is None
