...
def upgrade() -> None:
    ...
    for table in _public_tables:
        op.execute(f"ALTER TABLE IF EXISTS {table} OWNER TO praedixa_owner;")

    # ── Public schema ownership + grants ────────────
    # Transfer public schema ownership to praedixa_owner.
    op.execute("ALTER SCHEMA public OWNER TO praedixa_owner;")
    ...
    op.execute(
        "GRANT SELECT ON ALL TABLES IN SCHEMA public TO praedixa_platform_reader"
    )
    op.execute(
        "GRANT SELECT, INSERT, UPDATE, DELETE"
