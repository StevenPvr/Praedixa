...
Dans `HeroSection.tsx`, la signature de marque doit reutiliser `PraedixaLogo` depuis `@praedixa/ui` avec le mot-symbole `Praedixa`, et vivre dans la colonne droite desktop si elle remplace la carte laterale.
...
Sur la hero video actuelle, la colonne droite doit rester reservee a un seul signal fort: soit une carte de preuve, soit la signature `Praedixa`, jamais les deux.
Quand un bloc remplace la carte de preuve a droite du hero video, l'ancrer dans la colonne desktop avec un wrapper `relative` stable et sans gros `translate-y`; il doit sembler suspendu a droite, pas tombe en bas du viewport.
