...
Pour les mots accentues du hero, choisir la couleur de marque la plus lisible sur le media reel (`--accent-*` ou `--brass-*`), pas une teinte pale qui ressemble a un simple halo.
...
Dans `ClosedLoopTeaserSection.tsx`, garder la boucle produit complete: `Federer -> Predire -> Calculer -> Declencher -> Prouver`.
