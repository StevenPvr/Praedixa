...
function KnowledgeSections({
  ...
  page: ReturnType<typeof getKnowledgePage>;
}) {
  return (
...
function KnowledgeRelatedContent({
  links,
  locale,
}: {
  links: NonNullable<ReturnType<typeof getKnowledgePage>["links"]>;
  locale: Locale;
}) {
  if (!links.length) {
    return null;
    ...
    <div className="mt-12 rounded-xl border border-border-subtle bg-neutral-50/50 p-6">
      ...
      <ul className="mt-3 list-none space-y-2 p-0">
        {links.map((link) => {
          ...
          const href = slugs ? `/${locale}/${slugs[locale]}` : `/${locale}`;
          return (
...
function KnowledgePageActions({
  ...
  proofHref: string;
  scopingHref: string;
  useExamplePrimary: boolean;
  useProofSecondary: boolean;
  valueProp: ReturnType<typeof getValuePropContent>;
}) {
  return (
    <div className="mt-12 flex flex-wrap items-center gap-3 border-t border-border-subtle pt-8">
      <Link
        href={useExamplePrimary ? proofHref : scopingHref}
...
export function KnowledgePage({
  ...
  const sectionAnchors = buildSectionAnchors(page);
  ...
  return (
    <SectionShell className="py-16 md:py-24">
      ...
      <article className={articleClassName}>
        {showStickySubnav ? (
          <KnowledgePageSideNav items={sectionAnchors} locale={locale} />
        ) : null}
        ...
        <div>
          ...
          <KnowledgePageActions
            ...
            useExamplePrimary={useExamplePrimary}
            useProofSecondary={useProofSecondary}
            valueProp={valueProp}
          />
        </div>
      </article>
