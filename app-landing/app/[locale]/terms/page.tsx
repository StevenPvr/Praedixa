...
const FR_PATH = `/fr/${localizedSlugs.terms.fr}`;
const EN_PATH = `/en/${localizedSlugs.terms.en}`;
...
export default async function TermsPage({
  ...
  const locale = await resolveLocale(params);
  const expected = localizedSlugs.terms[locale];

  if (expected !== "terms") {
    permanentRedirect(`/${locale}/${expected}`);
  }

  return <LegalStaticPage locale={locale} pageKey="terms" />;
}
