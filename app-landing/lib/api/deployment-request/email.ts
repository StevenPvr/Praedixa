...
function buildAdminHtml(data: {
  ...
  const attribution = data.acquisitionSource
    ...
    : na;
  return `
    <h2>Nouvelle demande de déploiement Praedixa</h2>
  ...
  return `
    <div style="font-family: Arial, sans-serif; max-width: 620px; margin: 0 auto; color: ${EMAIL_COLORS.body};">
      ...
      <p style="font-size: 16px; line-height: 1.6;">
        Nous confirmons la bonne réception de votre demande de déploiement Praedixa.
      </p>
...
export async function sendDeploymentRequestEmails(
  resend: Resend,
  data: ValidatedData,
  ip: string,
): Promise<void> {
  ...
  const [adminResult, confirmResult] = await Promise.all([
    resend.emails.send({
      ...
      subject: `Nouvelle demande de déploiement - ${subjectCompany}`,
    ...
    resend.emails.send({
      from: fromEmail,
      to: [data.email],
      subject: "Demande de déploiement reçue - Praedixa",
