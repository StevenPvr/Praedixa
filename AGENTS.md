...
Before claiming a GitHub Actions workflow-action ref fix is done, search every workflow and confirm the remote PR head SHA is running the updated YAML; a stale failed run is not proof that the current branch still carries the bad ref.
Before continuing to iterate on noisy GitHub Actions failures, honor any explicit user request to disable GitHub Actions by disabling the workflow definitions themselves and stopping further CI reruns.
