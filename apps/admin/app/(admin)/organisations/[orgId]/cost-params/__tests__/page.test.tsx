...
import OrgCostParamsPage from "../page";

describe("OrgCostParamsPage", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("renders data table with records", () => {
    mockUseApiGetPaginated.mockReturnValue({
      data: [{ id: "1", paramType: "overtime_hourly", value: 25.5 }],
      total: 1,
      loading: false,
      error: null,
      refetch: vi.fn(),
  ...
  it("shows record count", () => {
    ...
    render(<OrgCostParamsPage />);
    expect(screen.getByText("15 parametres")).toBeInTheDocument();
  });
});
