...
class TestSensitiveFieldCompleteness:
    ...
    @pytest.mark.xfail(
        reason="P1 gap: recurrence_pattern not yet in _SENSITIVE_FIELDS — "
        "reveals chronic condition patterns for medical absences",
        strict=True,
    )
    ...
    @pytest.mark.xfail(
        reason="P1 gap: recurrence_pattern not yet removed by mask_medical_reasons",
        strict=True,
    )
...
class TestTypeResolutionEdgeCases:
    ...
    def test_enum_object_type_masked(self) -> None:
        ...
        absences = [{"type": AbsenceType.SICK_LEAVE, "reason": "Flu"}]
        result = mask_medical_reasons(absences)
        assert result[0]["reason"] == _MASKED_REASON
