...
class TestSanitizeText:
    ...
    def test_encodes_html_and_limits(self):
        result = sanitize_text("<script>alert('xss')</script>", max_length=50)
        assert "<script>" not in result
        assert "&lt;script&gt;" in result

    def test_truncation_after_encoding(self):
...
class TestValidateUuid:
    ...
    def test_error_does_not_reflect_input(self):
...
class TestValidateHorizon:
    ...
    def test_invalid_1_raises(self):
        with pytest.raises(ValueError, match="Invalid horizon"):
            validate_horizon(1)

    def test_invalid_30_raises(self):
        with pytest.raises(ValueError, match="Invalid horizon"):
            validate_horizon(30)

    def test_invalid_0_raises(self):
        with pytest.raises(ValueError, match="Invalid horizon"):
            validate_horizon(0)
