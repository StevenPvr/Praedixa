...
class TestValidateXlsxMagic:
    ...
    def test_csv_content_rejected(self):
    ...
    def test_four_bytes_with_pk_passes(self):
...
class TestDetectEncoding:
    ...
    def test_empty_string_encoding_raises(self):
        fake_result = {"encoding": "", "confidence": 0}
        with patch("app.services.file_parser.chardet.detect", return_value=fake_result):
            with pytest.raises(FileParseError, match="Could not detect"):
                _detect_encoding(b"bytes")

    def test_iso_8859_1_normalizes_to_latin1(self):
        fake_result = {"encoding": "ISO-8859-1", "confidence": 0.99}
        with patch("app.services.file_parser.chardet.detect", return_value=fake_result):
            assert _detect_encoding(b"some bytes") == "latin-1"

    def test_windows_1252_normalizes_to_cp1252(self):
    ...
    def test_utf7_rejected(self):
        fake_result = {"encoding": "UTF-7", "confidence": 0.90}
        with patch("app.services.file_parser.chardet.detect", return_value=fake_result):
            with pytest.raises(FileParseError, match="Unsupported encoding"):
...
class TestDetectDelimiter:
    ...
    def test_equal_consistency_last_delimiter_wins(self):
    ...
    def test_semicolon_wins_with_higher_header_count(self):
    ...
    def test_no_delimiters_at_all(self):
        text = "name\nAlice\nBob"
        assert _detect_delimiter(text) == ";"

    def test_more_than_10_lines_uses_first_10(self):
...
class TestSanitizeCellValue:
    ...
    def test_equals_then_minus_number(self):
        """=-42 -> first loop strips '=', then '-42' -> '-' followed by '4' (digit),
        so preserved."""
        assert _sanitize_cell_value("=-42") == "-42"

    def test_prefix_in_middle_not_stripped(self):
        assert _sanitize_cell_value("a=b+c") == "a=b+c"

    def test_only_formula_chars_no_minus(self):
        assert _sanitize_cell_value("=+@") == ""
...
class TestCoerceValue:
    ...
    def test_french_boolean(self, input_val, expected):
        warnings: list[str] = []
    ...
    def test_numeric_string_not_french_decimal(self):
        warnings: list[str] = []
        result = _coerce_value("1234.56", "amount", warnings)
        assert result == "1234.56"

    def test_iso_date_not_matched(self):
        warnings: list[str] = []
...
class TestCoerceRow:
    ...
    def test_mixed_types_in_one_row(self):
        warnings: list[str] = []
        result = _coerce_row(
...
class TestDetectFormat:
    ...
    def test_csv_fallback_empty_ext(self):
...
class TestParseCsv:
    ...
    def test_max_rows_enforcement(self):
    ...
    def test_formula_injection_stripped(self):
        text = "name;formula\nAlice;=SUM(A1)\n"
        rows, _ = _parse_csv(text, max_rows=1000)
    ...
    def test_whitespace_cell_becomes_none(self):
        text = "a;b\nvalue;   \n"
        rows, _ = _parse_csv(text, max_rows=1000)
        assert rows[0]["b"] is None
...
class TestParseXlsx:
    ...
    def test_none_header_becomes_unnamed(self):
        wb = Workbook()
        ws = wb.active
...
class TestOpenXlsxWorksheet:
    ...
    def test_missing_sheet_lists_available(self):
        content = _make_xlsx_bytes(["col"], [[1]])
        with pytest.raises(FileParseError, match="Available.*Sheet1"):
...
class TestParseFileCsv:
    def test_basic_csv_semicolon(self):
    ...
    def test_csv_only_newlines(self):
    ...
    def test_csv_max_rows_explicit(self):
        lines = ["val\n"] + [f"{i}\n" for i in range(100)]
    ...
    def test_csv_max_rows_from_settings(self):
        lines = ["a;b\n"] + [f"{i};{i}\n" for i in range(20)]
        content = _make_csv_bytes("".join(lines))
    ...
    def test_csv_none_for_empty_cells(self):
        content = _make_csv_bytes("a;b;c\n1;;3\n")
        result = parse_file(content, "data.csv")
        assert result.rows[0]["b"] is None

    def test_csv_with_fewer_cols_than_header(self):
        content = _make_csv_bytes("a;b;c\n1;2\n")
        result = parse_file(content, "data.csv")
...
class TestParseFileXlsx:
    ...
    def test_xlsx_duplicate_columns(self):
        content = _make_xlsx_bytes(["nom", "nom"], [[1, 2]])
        with pytest.raises(FileParseError, match="Duplicate"):
            parse_file(content, "data.xlsx")

    def test_xlsx_empty_rows_skipped(self):
        content = _make_xlsx_bytes(["val"], [[1], [None], [3]])
        result = parse_file(content, "data.xlsx")
        assert result.row_count == 2

    def test_xlsx_none_header_gets_unnamed(self):
        wb = Workbook()
        ws = wb.active
    ...
    def test_xlsx_corrupted_after_magic(self):
        content = b"PK\x03\x04" + b"\x00" * 100
        with pytest.raises(FileParseError, match="Failed to open XLSX"):
            parse_file(content, "data.xlsx")

    def test_xlsx_no_active_sheet(self):
        wb = Workbook()
        buf = io.BytesIO()
        wb.save(buf)
        content = buf.getvalue()
...
class TestParseFileFormatHint:
    ...
    def test_no_extension_falls_to_csv_path(self):
        content = _make_csv_bytes("a;b\n1;2\n")
