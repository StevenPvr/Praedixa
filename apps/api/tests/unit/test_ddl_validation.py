...
class TestValidateIdentifier:
    ...
    def test_valid_identifier_at_max_length(self):
    ...
    def test_rejects_reserved_word_select(self):
        with pytest.raises(DDLValidationError, match="reserved word"):
            validate_identifier("select")

    def test_rejects_reserved_word_table(self):
        with pytest.raises(DDLValidationError, match="reserved word"):
            validate_identifier("table")

    def test_rejects_reserved_word_from(self):
        with pytest.raises(DDLValidationError, match="reserved word"):
    ...
    def test_rejects_sql_prefix(self):
        with pytest.raises(DDLValidationError, match="reserved prefix"):
            validate_identifier("sql_query")

    def test_rejects_praedixa_prefix(self):
        with pytest.raises(DDLValidationError, match="reserved prefix"):
            validate_identifier("praedixa_internal")

    def test_rejects_platform_prefix(self):
        with pytest.raises(DDLValidationError, match="reserved prefix"):
            validate_identifier("platform_config")

    def test_rejects_audit_prefix(self):
        with pytest.raises(DDLValidationError, match="reserved prefix"):
            validate_identifier("audit_log")

    def test_rejects_public_prefix(self):
        with pytest.raises(DDLValidationError, match="reserved prefix"):
...
class TestValidateClientSlug:
    ...
    def test_valid_slug_min_length_3(self):
    ...
    def test_rejects_too_short_2_chars(self):
        with pytest.raises(DDLValidationError, match="3-35"):
            validate_client_slug("ab")

    def test_rejects_too_long_36_chars(self):
...
class TestValidateSchemaName:
    ...
    def test_valid_transformed_schema(self):
        assert validate_schema_name("acme_transformed") == "acme_transformed"

    def test_allows_praedixa_prefix(self):
    ...
    def test_rejects_information_schema_prefix(self):
        with pytest.raises(DDLValidationError, match="system-reserved prefix"):
...
class TestValidateColumnType:
    ...
    def test_all_allowed_types(self, input_type, expected_pg):
    ...
    def test_rejects_sql_injection_in_type(self):
...
class TestDDLValidationError:
    ...
    def test_error_has_422_status(self):
        err = DDLValidationError("test error")
