...
describe("ToastProvider", () => {
    ...
    return (
      <div>
        ...
        <button
          onClick={() => {
            for (let i = 0; i < 6; i++) {
              context?.addToast("info", `Toast ${i}`);
            }
          }}
          data-testid="add-many-toasts"
        >
          Add many
        </button>
      </div>
  ...
  it("removes oldest toasts when exceeding max capacity", () => {
    render(
      <ToastProvider>
        <TestConsumer />
      </ToastProvider>,
    ...
    act(() => {
      screen.getByTestId("add-many-toasts").click();
    });
  ...
  it("dismisses a toast via provider remove handler", () => {
    render(
      <ToastProvider>
        <TestConsumer />
      </ToastProvider>,
