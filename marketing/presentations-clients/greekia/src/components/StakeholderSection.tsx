...
export function StakeholderSection() {
  const { stakeholders } = greekiaMessaging;
  const [activeId, setActiveId] =
    useState<(typeof stakeholders.views)[number]["id"]>("brand");

  const firstView = stakeholders.views[0];
  if (!firstView) {
    return null;
  }

  const activeView =
    stakeholders.views.find((view) => view.id === activeId) ?? firstView;
