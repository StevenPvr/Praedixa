...
- Les liens internes automatiques s'appuient sur `marketing/content/internal-links.json`.
...
image: "/logo-full-920x400.png" # optionnel
...
Un modele de depart est disponible dans `marketing/content/blog/article-template.mdx`.
...
## 3bis) Hiérarchie de titres

- Le shell `BlogPostPage.tsx` rend deja le `title` en `H1`.
- Ne commencez pas le corps MDX par un `#` top-level.
- Le premier titre du corps doit commencer en `##` pour eviter un double `H1` en production.

## 4) Ajouter une règle de lien interne automatique

Le fichier `marketing/content/internal-links.json` pilote l'injection de liens contextuels.
